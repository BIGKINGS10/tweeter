  ...
  const $tweetHeader = $(`
    <header class="tweeted-header">
        <span class="header-image-name">
          <img src=${obj.user.avatars} alt="Osagie Omoruyi" class="tweet-header-image">
          ${obj.user.name}
        </span>
        <span>
  ...
  $(".tweet-form").submit(function(event) {
    ...
    if(data.length-5){
      if (data.replace(/\+/g, "").length -5){
        ...
        $(".tweet-label").text("Gotcha! You only put spaces in your tweet!");
      ...
      $(".tweet-label").text("You cannot tweet an empty message! lol");
      $(".tweet-form").addClass("error-message");
      setTimeout(function(){
        $(".tweet-label").text("What are you humming about?");
        $(".tweet-form").removeClass("error-message");
